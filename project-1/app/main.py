...
class Api(object):
    ...
    def contact_us(self, name, email, phone, message):
        with sqlite3.connect("db/db.sqlite3") as conn:
            c = conn.cursor()
            c.execute("INSERT INTO site_contact_us (name, email, phone, message) VALUES ('%s', '%s', '%s', '%s')" % (name, email, phone, message))
            conn.commit()
            return {"status": True}
    ...
    def review(self, name, email, message):
        with sqlite3.connect("db/db.sqlite3") as conn:
            c = conn.cursor()
            c.execute("INSERT INTO site_reviews (name, email, message) VALUES ('%s', '%s', '%s')" % (name, email, message))
            conn.commit()
            return {"status": True}
    ...
    def query_services(self, query):
        with sqlite3.connect("db/db.sqlite3") as conn:
            c = conn.cursor()
            c.execute("SELECT * FROM site_services WHERE name LIKE '%s%%'" % (query))
            result = c.fetchall()
            return json.dumps(result)
